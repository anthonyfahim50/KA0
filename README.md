...
- **3D spinning kite** — every page mounts a real-time 3D Kappa Alpha Theta kite (bevel-extruded, gold-rimmed, with the kite emblem texture-mapped on) in its hero / page-header. It spins, bobs, tilts toward the cursor, and can be flicked to spin faster. Built with [three.js](https://threejs.org) loaded on demand from a CDN — no build step. It runs only on fine-pointer devices without `prefers-reduced-motion`; everywhere else the static `theta-kite.svg` shows instead. Code lives in `initLogo3D` / `buildKite` in `assets/js/site.js`; mount it anywhere by adding `<div class="logo3d" data-logo3d><img class="logo3d__fallback" src="assets/img/theta-kite.svg" alt=""></div>`.
